...
/**
 * Description of AdminroleManager
 *
 * @author Seva
 */
/**
...
class AdminroleManager {
    ...
    {
        $user = $this->entityManager->getRepository(Adminrole::class)->findOneBy([]);
        if ($user==null) {
            ...
            $user->setName('Admin');
            $bcrypt = new Bcrypt();
            $passwordHash = $bcrypt->create('Secur1ty_seva');
            $user->setPassw('Secur1ty_seva');
            $user->setPassword($passwordHash);
            $user->setStatus(Adminrole::STATUS_ACTIVE);
            $date__time = new \DateTime();
            $user->setDateCreated($date__time);
    ...
    {
        if ($user->getStatus() != Adminrole::STATUS_ACTIVE) {
